...
def test_collaboration_final():
    ...
    try:
        ...
        can_edit = team_manager.has_permission(team.id, "final_test_owner", Permission.EDIT_PROJECT)
        can_invite = team_manager.has_permission(team.id, "final_test_owner", Permission.INVITE_MEMBERS)
        print(f"   ✅ Permissions: edit={can_edit}, invite={can_invite}")
        
        analytics = team_manager.get_team_analytics(team.id, "final_test_owner")
        print(f"   ✅ Analytics: {analytics['total_members']} members, {analytics['activity_rate']}% activity")
        ...
        if project_result["success"]:
            ...
            # Test user dashboard
            dashboard_result = collaboration_platform.get_user_dashboard("final_test_owner")
