...
class MetricsCollector:
    ...
    def _store_metric_db(self, name: str, timestamp: datetime, value: float, 
                        labels: Dict[str, str], metadata: Dict[str, Any]):
    ...
    def check_alerts(self, metrics_collector: MetricsCollector):
        ...
        for alert_name, alert in self.alerts.items():
            ...
            try:
                ...
                metric_data = metrics_collector.get_metric(metric_name, timedelta(minutes=5))
                if not metric_data:
                    continue
...
"""
        ...
        for name, status in dashboard_data["health_status"].items():
            status_emoji = "✅" if status["status"] == "healthy" else "⚠️" if status["status"] == "timeout" else "❌"
        ...
        if dashboard_data["active_alerts"]:
            ...
            for alert in dashboard_data["active_alerts"]:
                ...
                report += f"- {severity_emoji} **{alert['name']}**: {alert['message']}\n"
