        ...
        try:
            ...
            for op_name, op_metrics in metrics.get("operations", {}).items():
                ...
                if avg_time > 10:  # More than 10 seconds average
                    suggestions.append(f"⚠️ {op_name} is slow (avg: {avg_time:.2f}s) - consider optimization")
                
                if max_time > 60:  # More than 60 seconds max
                    suggestions.append(f"🚨 {op_name} has very slow operations (max: {max_time:.2f}s) - investigate bottlenecks")
            
            # Analyze system usage
            cpu_usage = metrics.get("cpu_usage", [])
            if cpu_usage:
                avg_cpu = sum(stat["cpu_percent"] for stat in cpu_usage) / len(cpu_usage)
                if avg_cpu > 80:
                    suggestions.append(f"🔥 High CPU usage detected (avg: {avg_cpu:.1f}%) - consider reducing load")
            
            memory_usage = metrics.get("memory_usage", [])
            if memory_usage:
                avg_memory = sum(stat["percent"] for stat in memory_usage) / len(memory_usage)
                if avg_memory > 85:
                    suggestions.append(f"💾 High memory usage detected (avg: {avg_memory:.1f}%) - consider memory optimization")
            
            # Check for potential bottlenecks
            if not suggestions:
                suggestions.append("✅ Performance looks good! No major issues detected.")
