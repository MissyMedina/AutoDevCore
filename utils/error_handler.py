    ...
    def handle_error(
        ...
        if error_type in self.recovery_strategies:
            ...
            if recovery_result["success"]:
                ...
                self.logger.warning(
                    f"Recovery failed for {error_type} error: {recovery_result['reason']}"
                )
    ...
    def __call__(self, func: Callable):
        def wrapper(*args, **kwargs):
            ...
            self.error_handler.handle_error(
                final_error,
                {"function": func.__name__, "attempts": self.max_retries + 1},
            )
...
def safe_execute(func: Callable, *args, **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        error_response = error_handler.handle_error(
            error, {"function": func.__name__, "args": str(args), "kwargs": str(kwargs)}
        )

        return {"success": False, "result": None, "error": error_response}
...
def _generate_error_recommendations(stats: Dict[str, Any]) -> List[str]:
    ...
    if stats["total_errors"] > 10:
        recommendations.append(
            "High error rate detected - review error logs and fix root causes"
        )

    if stats.get("GPT_OSS_ERROR", 0) > 5:
        recommendations.append(
            "Frequent GPT-OSS errors - check model availability and network connectivity"
        )

    if stats.get("TIMEOUT_ERROR", 0) > 3:
        recommendations.append(
            "Timeout errors detected - consider increasing timeouts or optimizing operations"
        )
