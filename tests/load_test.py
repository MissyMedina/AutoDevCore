...
class PerformanceMonitor:
    ...
    def _get_endpoint_stats(self) -> Dict[str, Any]:
        ...
        for name, stats in endpoint_stats.items():
            stats['avg_response_time_ms'] = (stats['total_time'] / stats['count']) * 1000
...
def run_load_test_scenario(scenario_name: str, users: int, spawn_rate: int, duration: int) -> Dict[str, Any]:
...
def generate_load_test_report(results: Dict[str, Any]) -> str:
    ...
    report.append(f"- **Average Throughput:** {results['summary']['avg_throughput']:.2f} RPS")
    report.append("")
    ...
    for scenario in results['scenarios']:
        ...
        report.append(f"- **Total Requests:** {scenario['summary']['total_requests']:,}")
        report.append(f"- **Error Rate:** {scenario['summary']['error_rate']:.2%}")
        report.append(f"- **Throughput:** {scenario['summary']['throughput_rps']:.2f} RPS")
        report.append(f"- **Avg Response Time:** {scenario['summary']['avg_response_time_ms']:.2f}ms")
        report.append(f"- **P95 Response Time:** {scenario['percentiles']['p95_ms']:.2f}ms")
        report.append("")
    ...
    worst_response_time = max(scenarios, key=lambda x: x['summary']['avg_response_time_ms'])
    
    report.append(f"**Best Throughput:** {best_throughput['scenario']} ({best_throughput['summary']['throughput_rps']:.2f} RPS)")
    report.append(f"**Slowest Response:** {worst_response_time['scenario']} ({worst_response_time['summary']['avg_response_time_ms']:.2f}ms)")
    report.append("")
    
    # Recommendations
    report.append("## 💡 Recommendations")
    
    avg_error_rate = results['summary']['total_errors'] / results['summary']['total_requests']
    if avg_error_rate > 0.05:
        report.append("- ⚠️ **High error rate detected** - Review error handling and system stability")
