...
class AIOptimizer:
    ...
    def get_optimal_model(self, task_type: str, complexity: str = "medium") -> ModelConfig:
    ...
    def record_performance(self, model_type: ModelType, task_type: str, 
...
"""
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "optimization_stats": {
                "prompts_optimized": sum(1 for req in self.request_history if req.get("optimized")),
                "fallbacks_used": sum(1 for req in self.request_history if req.get("fallback")),
                "avg_response_time": sum(req.get("duration", 0) for req in self.request_history) / len(self.request_history) if self.request_history else 0
            }
        }
...
def optimize_ai_request(task_type: str, complexity: str = "medium"):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if 'prompt' in kwargs:
                kwargs['prompt'] = ai_optimizer.optimize_prompt(kwargs['prompt'], task_type)
            ...
            try:
                ...
                # Record success
                ai_optimizer.record_performance(model_config.type, task_type, duration, True)
                ...
                # Record failure
                ai_optimizer.record_performance(model_config.type, task_type, duration, False)
