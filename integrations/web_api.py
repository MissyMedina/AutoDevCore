...
class AutoDevCoreAPI:
    ...
    async def health_check(self, request):
        ...
        return web.json_response(
            {"status": "healthy", "service": "AutoDevCore API", "version": "1.0.0"}
        )
    ...
    async def register(self, request):
        ...
        try:
            ...
            if not all([username, password, email]):
                return web.json_response(
                    {"error": "Username, password, and email required"}, status=400
                )
    ...
    async def refresh_token(self, request):
        ...
        try:
            ...
            return web.json_response(
                {"error": f"Token refresh failed: {str(e)}"}, status=500
            )
    ...
    async def get_profile(self, request):
        ...
        try:
            ...
            if not auth_header or not auth_header.startswith("Bearer "):
                return web.json_response(
                    {"error": "Authorization header required"}, status=401
                )
            ...
            return web.json_response(
                {"error": f"Profile retrieval failed: {str(e)}"}, status=500
            )
    ...
    async def get_status(self, request):
        ...
        try:
            ...
            return web.json_response(
                {"error": f"Status retrieval failed: {str(e)}"}, status=500
            )
