...
class MetricsStorage:
    ...
    def store_metric(self, name: str, value: float, labels: Optional[Dict[str, str]] = None, 
    ...
    def store_performance_operation(self, operation_name: str, start_time: datetime, 
                                  end_time: Optional[datetime] = None, duration: Optional[float] = None,
                                  status: str = "running", metadata: Optional[Dict[str, Any]] = None) -> bool:
    ...
    def store_health_check(self, check_name: str, status: str, details: Optional[str] = None,
    ...
    def get_metrics(self, metric_name: Optional[str] = None, hours: int = 24) -> List[Dict[str, Any]]:
    ...
    def get_health_status(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"overall_status": "unknown", "checks": {}, "timestamp": datetime.now().isoformat()}
    ...
    def cleanup_old_data(self, days: int = 30) -> bool:
        ...
        try:
            ...
            cursor.execute("DELETE FROM performance_ops WHERE start_time < datetime('now', '-{} days')".format(days))
            
            # Delete old alerts (keep unresolved ones)
            cursor.execute("DELETE FROM alerts WHERE resolved = 1 AND timestamp < datetime('now', '-{} days')".format(days))
            
            # Delete old health checks
            cursor.execute("DELETE FROM health_checks WHERE timestamp < datetime('now', '-{} days')".format(days))
    ...
    def export_data(self, format: str = "json", hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            if format == "json":
                ...
                return {"csv_data": "timestamp,metric_name,value\n" + "\n".join([
