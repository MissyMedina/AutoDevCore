...
class PerformanceMonitor:
    ...
    def start_operation(self, operation_name: str) -> str:
        ...
        self.operation_times[operation_name].append(
            {"id": operation_id, "start_time": time.time(), "status": "running"}
        )
        return operation_id

    def end_operation(
        self, operation_name: str, operation_id: str, success: bool = True
    ):
    ...
    def get_operation_stats(self, operation_name: str = None) -> Dict[str, Any]:
        ...
        if completed_ops:
            avg_duration = sum(op.get("duration", 0) for op in completed_ops) / len(
                completed_ops
            )
            ...
            "success_rates": {
                name: (
                    (
                        ...
                        / (self.success_counts[name] + self.error_counts[name])
                        * 100
                    )
                    if (self.success_counts[name] + self.error_counts[name]) > 0
                    else 0
                )
                for name in set(
                    list(self.success_counts.keys()) + list(self.error_counts.keys())
                )
            },
        }
...
"""
    ...
    def print_summary(self):
        ...
        print("=" * 50)
        print(
            f"Memory: {stats['memory']['current_mb']} MB (avg: {stats['memory']['avg_mb']} MB)"
        )
        print(
            f"CPU: {stats['cpu']['current_percent']}% (avg: {stats['cpu']['avg_percent']}%)"
        )
        print()
        ...
        for op_name, op_stats in stats["operations"].items():
            print(
                f"  {op_name}: {op_stats['avg_duration_seconds']}s avg, {op_stats['success_rate_percent']}% success"
            )
