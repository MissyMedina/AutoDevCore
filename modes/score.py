...
class ScoreMode(BaseMode):
    ...
    def __init__(self, app_dir: str, template: str, output_dir: str, verbose: bool = False):
    ...
    def _analyze_file_structure(self) -> Dict[str, Any]:
        ...
        for item in self.app_dir.rglob("*"):
            if item.is_file():
                ...
                elif item.name in ["requirements.txt", "package.json", "Dockerfile", ".env"]:
        ...
        try:
                ...
                try:
                    with open(file_path, 'r', encoding='utf-8') as f:
                        ...
                        if any(keyword in content.lower() for keyword in 
                               ['password', 'secret', 'token', 'auth', 'encrypt', 'hash']):
            ...
            try:
                ...
                if start != -1 and end != 0:
                    ...
                    return {"security_score": 50, "security_concerns": ["Unable to analyze"]}
            except:
                return {"security_score": 50, "security_concerns": ["Analysis failed"]}
                
        except Exception as e:
            return {"security_score": 50, "security_concerns": [f"Analysis error: {str(e)}"]}
    ...
    def _fallback_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "security_analysis": {"security_score": 50, "security_concerns": ["Analysis unavailable"]},
            "performance_analysis": {"performance_score": 50, "performance_notes": ["Analysis unavailable"]},
            "architecture_analysis": {"architecture_score": 50, "patterns_detected": [], "recommendations": []}
        }
    
    def _evaluate_app_intelligent(self, template_data: dict, code_analysis: dict) -> dict:
        ...
        for category, criteria in categories.items():
            ...
            for criterion in criteria:
                max_score = criterion.get("max_score", 1) if criterion.get("type") == "scale" else 1
                category_max += max_score
                score = self._evaluate_criterion_intelligent(criterion, code_analysis, category)
        ...
        scores["recommendations"] = self._generate_recommendations(scores, code_analysis)
        
        return scores
    
    def _evaluate_criterion_intelligent(self, criterion: dict, code_analysis: dict, category: str) -> int:
    ...
    def _generate_recommendations(self, scores: dict, code_analysis: dict) -> List[str]:
        ...
        for category, data in scores["categories"].items():
            if data["percentage"] < 60:
                recommendations.append(f"Improve {category.lower()} - current score: {data['percentage']:.1f}%")
        ...
        with open(report_file, 'w') as f:
            ...
            if "overall_quality_score" in code_quality:
                ...
                if "strengths" in code_quality and code_quality["strengths"]:
                    f.write(f"- **Strengths**: {', '.join(code_quality['strengths'][:3])}\n")
                if "weaknesses" in code_quality and code_quality["weaknesses"]:
                    f.write(f"- **Areas for Improvement**: {', '.join(code_quality['weaknesses'][:3])}\n")
                f.write("\n")
            ...
            if "security_score" in security:
                ...
                if "security_concerns" in security and security["security_concerns"]:
                    f.write(f"- **Concerns**: {', '.join(security['security_concerns'][:3])}\n")
                f.write("\n")
            ...
            f.write(f"- **JavaScript Files**: {file_structure.get('javascript_files', 0)}\n")
            f.write(f"- **Configuration Files**: {file_structure.get('config_files', 0)}\n")
