...
class APIConfigPanel:
    ...
    def show_provider_config(self, provider: str):
        ...
        with st.form(key=f"config_{provider}"):
            ...
            default_model = provider_config.get('default_model', provider_info['models'][0])
            selected_model = st.selectbox(
                "Default Model",
                options=provider_info['models'],
                index=provider_info['models'].index(default_model) if default_model in provider_info['models'] else 0,
                ...
                if api_key:
                    ...
                    if self.save_config():
                        ...
                        with st.spinner("Testing connection..."):
                            result = self.test_api_connection(provider, api_key)
                            if result['success']:
                                st.success(f"✅ Connection verified: {result['message']}")
                            else:
                                st.warning(f"⚠️ Configuration saved but connection failed: {result['message']}")
                    else:
    ...
    def show_api_panel(self):
        ...
        # Provider tabs
        tab_names = [f"{info['icon']} {info['name']}" for info in self.providers.values()]
        ...
        with col1:
            ...
            strategy = st.selectbox(
                ...
                index=['auto', 'cost_optimized', 'performance_optimized', 'manual'].index(
                    self.config.get('global', {}).get('strategy', 'auto')
                ),
