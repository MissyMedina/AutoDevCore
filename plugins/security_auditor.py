...
class SecurityAuditor:
    ...
    def _check_dangerous_patterns(self, content: str, file_path: str):
        ...
        dangerous_patterns = {
            ...
            r"subprocess\.call\(.*shell=True": (
                ...
                "CWE-78",
                "Use shell=False or list arguments",
            ),
            r"os\.system\(": (
            ...
            r"pickle\.loads\(": (
                ...
                "CWE-502",
                "Use json or other safe serialization",
            ),
            r"\binput\(": (
        ...
        if any(
            template_dir in file_path
            for template_dir in ["demo_output", "output", "templates", "config"]
        ):
            return
        ...
        for pattern, (title, cwe, recommendation) in dangerous_patterns.items():
            ...
            for match in matches:
                ...
                if (
                    "#" in context[: context.find(match.group())]
                    or '"""' in context
                    or "'''" in context
                ):
                    continue
                ...
                if pattern == r"sqlite3\.connect\(.*\)" and (
                    "monitoring" in file_path or "dashboard" in file_path
                ):
                    continue

                if pattern == r"\binput\(" and (
                    "help" in file_path or "cli" in file_path
                ):
                    continue
                        ...
                        if isinstance(target, ast.Name):
                            if any(
                                ...
                                if isinstance(node.value, ast.Constant):
                                    ...
                                    if (
                                        len(value_str) > 20
                                        and any(c.isupper() for c in value_str)
                                        and any(c.islower() for c in value_str)
                                        and any(c.isdigit() for c in value_str)
                                    ):
                                        if not self._is_in_comment_or_string(
                                            node, content
                                        ):
                                            self._add_issue(
                                                Severity.HIGH,
    ...
    def _audit_configuration(self):
        ...
        if gitignore.exists():
            ...
            has_python_bytecode = any(
                pattern in content for pattern in python_bytecode_patterns
            )
    ...
    def _audit_authentication(self):
        ...
        if req_file.exists():
            with open(req_file, "r") as f:
                requirements = f.read()
                if any(
                    hashlib in requirements
                    for hashlib in ["bcrypt", "passlib", "hashlib"]
                ):
                    has_hashing = True
    ...
    def _audit_owasp_compliance(self):
        ...
        if req_file.exists():
            with open(req_file, "r") as f:
                ...
                if any(
                    lib in requirements
                    for lib in ["pydantic", "cerberus", "marshmallow"]
                ):
                    has_validation = True
