...
class CollaborationManager:
    ...
    async def add_user_to_workspace(self, workspace_id: str, user_id: str, 
        ...
        async with self.lock:
            ...
            logging.info(f"User {user_id} added to workspace {workspace_id} with role {role.value}")
            return True
    ...
    async def update_project_data(self, workspace_id: str, user_id: str, 
        ...
        async with self.lock:
            ...
            logging.info(f"Project data updated in workspace {workspace_id} by user {user_id}")
            return True
    ...
    async def broadcast_message(self, workspace_id: str, message: CollaborationMessage):
        ...
        if workspace_id in self.message_history:
            ...
            if len(self.message_history[workspace_id]) > 100:
                self.message_history[workspace_id] = self.message_history[workspace_id][-100:]
    ...
    async def handle_message(self, user_id: str, workspace_id: str, data: Dict[str, Any]):
        ...
        try:
            ...
            if message_type == MessageType.PROJECT_UPDATE:
                ...
                success = await self.update_project_data(workspace_id, user_id, project_data)
                ...
                if workspace_id in self.workspaces and user_id in self.workspaces[workspace_id].users:
                    self.workspaces[workspace_id].users[user_id].cursor_position = cursor_data
                    self.workspaces[workspace_id].users[user_id].last_active = datetime.now()
    ...
    async def handle_user_disconnect(self, user_id: str, workspace_id: str):
        ...
        async with self.lock:
            ...
            if workspace_id in self.workspace_connections:
                if user_id in self.user_connections:
                    self.workspace_connections[workspace_id].discard(self.user_connections[user_id])
