    ...
    async def _check_gpt_oss_health(self, model_config: ModelConfig) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return any(
                    model_config.model_name in model.get("name", "") for model in models
                )
            return False
        except:
            return False
    ...
    def __init__(self):
        ...
        self.task_preferences = {
            TaskType.CODE_GENERATION: [
                ...
                ModelProvider.GPT_OSS,
            ],
            TaskType.APP_PLANNING: [
                ModelProvider.ANTHROPIC,
                ModelProvider.OPENAI,
                ModelProvider.GPT_OSS,
            ],
...
class MultiModelAI:
    ...
    async def generate(self, request: AIRequest) -> AIResponse:
        ...
        if not selected_model:
            # Use fallback model
            selected_model = next(
                m for m in self.models if m.provider == ModelProvider.FALLBACK
            )
        ...
        try:
            ...
            if (
                selected_model.provider != ModelProvider.FALLBACK
                and request.fallback_required
            ):
                fallback_model = next(
                    m for m in self.models if m.provider == ModelProvider.FALLBACK
                )
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "model_health": {
                model.model_name: model.is_available for model in self.models
            },
            "task_preferences": {
    ...
    async def test_multi_model():
        ...
        try:
            response = await generate_ai_response(
                prompt=prompt, task_type=TaskType.CODE_GENERATION, priority="normal"
            )
