...
class TeamManager:
    ...
    def __init__(self, data_dir: str = "data/teams"):
        ...
        self.role_permissions: Dict[TeamRole, Set[Permission]] = self._setup_role_permissions()
    ...
    def _load_data(self):
        ...
        try:
            ...
            logging.info(f"Loaded {len(self.teams)} teams and {len(self.invitations)} invitations")
    ...
    def _save_data(self):
        ...
        try:
            ...
            logging.info(f"Saved {len(self.teams)} teams and {len(self.invitations)} invitations")
        except Exception as e:
    ...
    def create_team(self, name: str, description: str, owner_id: str, 
                   owner_email: str, is_public: bool = False) -> Team:
        """Create a new team."""
    ...
    def add_member(self, team_id: str, user_id: str, username: str, email: str, 
                  role: TeamRole, added_by: str) -> bool:
    ...
    def update_member_role(self, team_id: str, user_id: str, new_role: TeamRole, 
                          updated_by: str) -> bool:
        ...
        if user_id in team.members:
            ...
            logging.info(f"Updated member {user_id} role to {new_role.value} in team {team_id}")
            return True
    ...
    def create_invitation(self, team_id: str, email: str, role: TeamRole, 
                         invited_by: str, expires_in_days: int = 7) -> Optional[TeamInvitation]:
        ...
        logging.info(f"Created invitation {invitation_id} for {email} to team {team_id}")
        return invitation
    
    def accept_invitation(self, invitation_id: str, user_id: str, username: str) -> bool:
    ...
    def get_pending_invitations(self, email: str) -> List[TeamInvitation]:
        ...
        return [
            ...
            if invitation.email == email and not invitation.is_accepted and datetime.now() <= invitation.expires_at
        ]
    ...
    def get_team_analytics(self, team_id: str, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        active_members = len([
            member for member in team.members.values()
...
def run(context=None):
    ...
    team_manager.add_member(
        team.id, "test_member_1", "Alice Developer", "alice@test.com",
        TeamRole.EDITOR, "test_owner_1"
    )
    
    team_manager.add_member(
        team.id, "test_member_2", "Bob Reviewer", "bob@test.com",
        TeamRole.VIEWER, "test_owner_1"
    )
    ...
    # Test permissions
    can_edit = team_manager.has_permission(team.id, "test_member_1", Permission.EDIT_PROJECT)
    can_invite = team_manager.has_permission(team.id, "test_member_1", Permission.INVITE_MEMBERS)
