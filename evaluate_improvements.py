    ...
    def analyze_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "avg_function_complexity": sum(self.function_complexities[-len([n for n in ast.walk(tree) if isinstance(n, ast.FunctionDef)]):]) / max(1, len([n for n in ast.walk(tree) if isinstance(n, ast.FunctionDef)]))
            }
...
class InterconnectednessAnalyzer:
    ...
    def calculate_coupling(self) -> Dict[str, float]:
        ...
        for file_path, imports in self.imports.items():
            ...
            internal_deps = sum(1 for imp in imports if not imp.startswith(('os', 'sys', 'json', 'time', 'datetime', 'pathlib', 'typing', 'unittest', 'pytest')))
...
def main():
    ...
    for file_path in python_files:
        ...
        if 'error' not in result:
            print(f"   📄 {file_path.name}: {result['functions']} functions, avg complexity: {result['avg_function_complexity']:.1f}")
    ...
    interconnectedness_score = interconnectedness_analyzer.get_overall_interconnectedness_score()
